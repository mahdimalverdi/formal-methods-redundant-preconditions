...
\paragraph{۱) \lr{IC} — بررسی استنتاج منطقی \lrpar{Implication Checking}}
...
\paragraph{۲) \lr{DC} — تحلیل وابستگی \lrpar{Dependency Calculation}}
...
\paragraph{۳) \lr{PC} — تحلیل گزاره‌ای/انتزاعی \lrpar{Predicate Checking}}
این روش یک بار تحلیل گزاره‌ای را اجرا می‌کند تا \lr{ARG} ساخته شود.
ایده‌ی کلیدی: اگر در یال متناظر با یک پیش‌شرط \lrpar{\texttt{assume(b)}}، گزاره‌ی انتزاعی قبل و بعد از آن تغییری نکند (یعنی اضافه شدن آن پیش‌شرط در محاسبه‌ی انتزاع اثر نگذارد)، آن پیش‌شرط در این اثبات «لازم» نبوده و می‌تواند زائد باشد.
مقاله برای نسخه‌ی مبتنی بر \lr{ARG} یک قضیه‌ی صحت \lrpar{soundness} بیان می‌کند: اگر عبور از یالِ \lr{assume} موجب تقویت predicate نشود، جایگزینی آن با \lr{\texttt{assume(true)}} همچنان شواهد کافی برای صحت قرارداد را حفظ می‌کند.
...
\subsection{مسئله}
در «Design by Contract» قراردادها معمولاً با مجموعه‌ای از پیش‌شرط‌ها \lrpar{Preconditions} و پس‌شرط‌ها \lrpar{Postconditions} نوشته می‌شوند.
هدف مقاله بررسی کیفیت قراردادهاست: آیا برخی پیش‌شرط‌ها \textbf{زائد} هستند (یعنی حتی بدون آن‌ها هم پس‌شرط‌ها برقرار می‌مانند)؟

\subsection{تعریف افزونگی \lrpar{Redundancy}}
...
مقاله علاوه بر افزونگی تکی، مفهوم \textbf{افزونگی گروهی} \lrpar{group-redundant} را هم تعریف می‌کند.

\subsection{روش‌های پیشنهادی مقاله}
مقاله سه تکنیک برای کشف پیش‌شرط‌های زائد ارائه می‌کند:
\begin{enumerate}
  \item \textbf{IC \lrpar{Implication Checking}:} بررسی می‌کند آیا \lr{\texttt{pre\_i}} به‌صورت منطقی از بقیه‌ی پیش‌شرط‌ها نتیجه می‌شود یا نه.
  \item \textbf{DC \lrpar{Dependency Calculation}:} روی CFA توسعه‌یافته، وابستگی‌های داده/کنترل بین \lr{\texttt{assume}}ها و \lr{\texttt{assert}}ها را بررسی می‌کند؛ اگر پیش‌شرط به هیچ پس‌شرطی وابسته نباشد، زائد است (روش نحوی/سینتکتیک).
  \item \textbf{PC \lrpar{Predicate Checking}:} با \lr{predicate analysis} و \lr{CEGAR} و ساخت \lr{ARG} بررسی می‌کند آیا در یالِ \lrcode{assume(b)}، predicate بعد از آن تغییر/تقویت می‌شود یا نه؛ اگر تقویت نشود، آن پیش‌شرط زائد است.
\end{enumerate}
...
\subsection{نتایج نمونه \lrpar{Example}}
...
\section{شبیه‌سازی \lrpar{Benchmark}}
...
\begin{enumerate}
  ...
  \item ساخت مجموعه مثال‌های واقع‌گرایانه و اجرای ابزار روی مثال‌ها و ثبت نتایج.
